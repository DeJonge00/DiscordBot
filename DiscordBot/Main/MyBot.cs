...
using DiscordBot.Main.Families;
...
namespace DiscordBot.Main
{
    class MyBot
    {
        ...
        private Families.Families families;
        ...
        public MyBot()
        {
            ...
            commands.CreateCommand("unacceptable")
                .Alias("unac")
                .Description("\n\tSend THE unacceptable gif")
                .Parameter("param", ParameterType.Unparsed)
                .Do(async (e) => await Unacceptable(e));
            ...
            discordClient.RoleUpdated += (s, e) =>
            {
                ...
                if (e.Before.Position != e.After.Position)
           ...
           if(Constants.user == "NYA")
            ...
            families = new Families.Families(commands);
        ...
        private async Task Hug(Discord.Commands.CommandEventArgs e)
        {
            ...
            if (e.Message.MentionedUsers.Count() <= 0 || e.Message.MentionedUsers.ElementAt(0).Id == e.User.Id)
            {
                ...
                else await e.Channel.SendMessage(e.User.Mention + Responses.hug[num] + e.GetArg("param"));
                return;
            }
        ...
        private async Task Ping(Discord.Commands.CommandEventArgs e)
        {
            ...
            await mess.Edit(mess.Text + "` Biri's ping: " + DateTime.Now.ToUniversalTime().Subtract(timediff).Milliseconds.ToString() + "`");
        }
        ...
        private async Task ServerInfo(Discord.Commands.CommandEventArgs e)
        {
            await e.Message.Delete();
            var message = "**ServerInfo:                     " + e.Server.Name + "**```";
            message += "\nIcon URL:              " + e.Server.IconUrl;
            message += "\nStart of the server:   " + e.Server.JoinedAt;
            message += "\nUsercount:             " + e.Server.UserCount;
            message += "\nChannelcount:          " + e.Server.ChannelCount;
            message += "\nDefault channel:       " + e.Server.DefaultChannel;
            message += "\nServer owner:          " + e.Server.Owner.Name;
            message += "\nServer region:         " + e.Server.Region.Name;
            message += "\nServer roles:          " + e.Server.RoleCount;
            foreach(Role r in e.Server.Roles)
                message += "\n\t\t(" + r.Members.Count() + " members)\t" + r.Name;
            foreach (String s in e.Server.Features)
                message += "\nFeature:               " + s;
            message += "```";
            await e.Channel.SendMessage(message);
        }
        ...
        private async Task Unacceptable(Discord.Commands.CommandEventArgs e)
        {
            await e.Message.Delete();
            await e.Channel.SendFile(@"Main\unacceptable.gif");
        }
        ...
        private async Task UserInfo(Discord.Commands.CommandEventArgs e)
        {
            ...
            message += "\nStatus:         ";
            if (e.User.Status.Value == UserStatus.Idle.Value)
                message += "Idle";
            if (e.User.Status.Value == UserStatus.DoNotDisturb.Value)
                message += "Do Not Disturb";
            if (e.User.Status.Value == UserStatus.Online.Value)
                message += "Online";
            if (e.User.Status.Value == UserStatus.Offline.Value)
                message += "Offline";
            ...
            {
                message += "\nNow playing:    " + FirstCharToUpper(user.CurrentGame.Value.Name);
            }
            message += "\nUser ID:        " + user.Id;
